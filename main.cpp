...
// --------------------------------------------------------
int emuSaveState(uint8_t* buffer, unsigned bufferSize) {
    unsigned actualSize = CPUWriteState(buffer, bufferSize);
    systemMessage("Saved state to buffer, size: %u", actualSize);
    return (int)actualSize;
}
int emuLoadState(const uint8_t* buffer, unsigned bufferSize) {
    bool success = CPUReadState(buffer, bufferSize);
    systemMessage("Loaded state from buffer, size: %u", bufferSize);
    return success ? 1 : 0;
}
// --------------------------------------------------------
